...
const bookShop =(()=>{
    let booksForRent= ['maths', 'physics', 'biology', 'chemistry', 'geology', 'commerce'];
   let rentedBooks=[];

   //  FUNCTION FOR RENTING A BOOK 

  function rentABook(bookId) {
     const currentAvailableBook=booksForRent.filter((_,index)=>index!=bookId);
...
let bookToReturn=rentedBooks[bookId];
    const booksYetToBeReturned=rentedBooks.filter((_,index)=>index!=bookId);
    if (rentedBooks.includes(bookToReturn)) {
        rentedBooks=booksYetToBeReturned;
        booksForRent.push(bookToReturn);
        return `\n <<< YOU HAVE  SUCCESSFULLY RETURNED '${bookToReturn}' TO OUR STORE>>>`
        }else{
...
//          });
//          console.log(bookShop.rentBooks(prompt(`\n choose the book you want to rent:\n ${bookAchieve} \n`)));
//     }
...
//                 return bookAchieve+= bookId+ ': '+books +'\n '
//             });
//             console.log(bookShop.returnedBook (prompt(`\n enter the index of the book to return \n ${bookAchieve}\n`)));
